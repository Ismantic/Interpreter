...
A/B replication of the **Translator** project (`../Translator/`) with two and
only two differences:

| | Translator baseline | Translator_piece |
|---|---|---|
...
Pipeline mirrors Translator: **SFT → CPO (LoRA) → GRPO (full-param)**.

## Setup

- venv: `/home/tfbao/new/HY-MT/.venv/bin/python -u` (shared with Translator;
...
bash run_grpo.sh from_sft     # from SFT directly → output_v18_grpo_from_sft/
                              #   (matches Translator/output_1.7b_grpo_sft_tuned lineage)
...
- **`train.py`** — SFT. Tokenizer-only fork of `../Translator/train.py`.
...
- **`train_cpo.py`** — CPO LoRA. Tokenizer-only fork of `../Translator/train_cpo.py`.
  `L_CPO = -log σ(β·(logπ(y_w) - logπ(y_l))) + λ·NLL(y_w)`, LoRA r=16
  all-linear. CPODataset built with piece chat IDs.
- **`merge_lora.py`** — port of `../Translator/merge_lora_qwen3.py`. Only diff:
...
- **`train_grpo.py`** — GRPO via TRL. Monkey-patches `vllm.LLM.__init__` to set
  ...
  penalty as Translator; eos forced to `</s>`.
...
- **`eval_vllm_piece.py`** — behavior-equivalent to `../Translator/eval_vllm.py`
...
## Comparison targets (from Translator/results.tsv)

| Stage | Translator ckpt | WMT23 zh→en (BLEU/COMET) | WMT23 en→zh |
|---|---|---|---|
