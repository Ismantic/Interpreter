...
Tokenizer-only fork of ../Qwen/train_grpo.py. The reward functions
...
Only differences from Qwen/train_grpo.py:
...
def comet_reward(prompts, completions, source, reference, **kwargs):
    """Reference-based COMET score as reward. Verbatim from Qwen."""
    global _reward_call
    comet = get_comet()
...
def repetition_penalty(prompts, completions, **kwargs):
    """4-gram unique ratio as penalty. Verbatim from Qwen."""
    rewards = []
