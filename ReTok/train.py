...
Tokenizer-only fork of ../Qwen/train.py — every hyperparameter, optimizer,
LR schedule, loss-mask boundary, and gradient-accumulation behavior is preserved
verbatim from the Qwen best-practice recipe (AdamW lr=2e-5, inverse_sqrt,
...
Usage (matches Qwen/train.py CLI 1:1):
    python train.py \\
...
class TranslationSFTDataset(Dataset):
    ...
    mirroring Qwen's behavior of supervising on `response + <|im_end|>`.
    """
    ...
    def __getitem__(self, index):
        ...
        # and supervised on response + eos, exactly like Qwen's prefix split.
