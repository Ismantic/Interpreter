...
### 为什么是 `grpo_full`（而不是 r2 / kiwi）

`results.tsv` 里记录过 3 个 GRPO 变体，磁盘上**只剩 `grpo_full`**：
...
### 非 ReTok 对照（Qwen baseline，用的是 Qwen3 原生 BPE）

不是这次要部署的目标，但作为分数对照：

| Stage | Qwen 项目 ckpt | WMT23 zh→en | WMT23 en→zh |
|---|---|---|---|
| SFT  | `Qwen/output_1.7b_base_v2`              | 21.58 / 0.7924 | 39.82 / 0.8556 |
| CPO  | `Qwen/output_1.7b_cpo_v3_plus_7b_merged`| 19.16 / 0.8017 | 32.69 / 0.8507 |
| GRPO | `Qwen/output_1.7b_grpo_sft_tuned`       | **22.85** / 0.8003 | **41.97** / 0.8540 |

ReTok BEST vs Qwen GRPO BEST 的差距：zh→en **−4.42 / −0.0036 COMET**，en→zh **−10.18 / −0.0029 COMET**。BLEU 跌得显著，COMET 接近。

### 文档之间的关系

- `ReTok/results.tsv` —— 所有 ReTok 实验的原始成绩表
- `ReTok/CLAUDE.md` —— 项目说明，标注了 BEST checkpoint
- `ReTok/RUN_BEST_MODEL.md` —— 本文档，部署/运行手册
...
下面是一段独立 Python，把它存成 `ReTok/quick_infer.py` 直接跑：
