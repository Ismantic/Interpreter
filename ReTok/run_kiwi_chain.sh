...
$PY -u eval/eval_vllm_piece.py \
    --model_path ./checkpoints/output_v18_tie_grpo_kiwi \
    --testset wmt23 --direction both \
    --max_model_len 1024 --gpu_memory_utilization 0.85 \
    > logs/eval_v18_tie_grpo_kiwi_wmt23.log 2>&1

echo "===== [3/3] update results.tsv $(date '+%H:%M') ====="
# 提取 BLEU / COMET
ZE_LINE=$(grep "zh-en:" logs/eval_v18_tie_grpo_kiwi_wmt23.log | tail -1)
EZ_LINE=$(grep "en-zh:" logs/eval_v18_tie_grpo_kiwi_wmt23.log | tail -1)
...
printf "piece_tie_grpo_kiwi\t./checkpoints/output_v18_tie_grpo_kiwi\twmt23\t%s\t%s\t%s\t%s\tGRPO with dual reward (COMET 1.0 + cometkiwi 1.0 + rep 0.3); from CPO-merged\n" \
