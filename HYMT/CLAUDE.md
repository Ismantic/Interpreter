...
Tokenizer-replacement (ReTok) experiment for the HY-MT1.5-1.8B translation model. The pipeline swaps the original HuggingFace tokenizer (vocab ~120K) for a custom SentencePiece-style "piece" tokenizer (vocab ~65K + special tokens), re-initializes the embedding matrix by mapping new tokens through the old tokenizer, then runs two training phases to recover (and ideally improve) zh↔en translation quality.

This is one of three sibling projects in the repo; see the repo-root `CLAUDE.md` for the map. All commands below are run **from this `HYMT/` directory** — every path is relative to it (model dirs, `private/`, `piece_mt.model` all live here). This is a Python ML codebase despite the C/C++-flavored `.gitignore` (legacy from the sibling Tokenizer repo).
...
### Auxiliary scripts (outside the standard Makefile flow)

- `distill.py` / `make_distill_sft.py` — generate teacher translations from a model and assemble them into SFT JSONL, for distilling the swapped-tokenizer model.
- `score_data_comet.py` — COMET-score a translation dataset and filter pairs below `--threshold` (default 0.80), for cleaning training data.
- `eval_bpb.py` — bits-per-byte eval on a pre-tokenized `.pt` file; a tokenizer-agnostic LM-quality metric to compare vocabularies fairly.
- `muon.py` / `aurora_opt.py` / `polar.py` — optimizer implementations (Muon is the one wired into `finetune_muon.py`; `aurora_opt.py`/`polar.py` are experimental Newton–Schulz / polar-factor variants).
