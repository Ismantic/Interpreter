...
eval_full() {
  local MODEL=$1
  $PY -u eval/eval_vllm.py --model_path $MODEL --testset wmt23 --direction both
}

eval_lora() {
  local LORA=$1
  local OUT=${2:-${LORA}_merged}
  $PY -u train/merge_lora_qwen3.py --base_model_path ./checkpoints/output_1.7b_base_v2 --adapter_model_path $LORA --output_path $OUT --save_dtype bf16
  $PY -u eval/eval_vllm.py --model_path $OUT --testset wmt23 --direction both
}

# === CPO + 7B chosen (LoRA) — Task #5 ===
cpo_7b_lora() {
  $PY -u train/train_cpo.py \
    --model_path ./checkpoints/output_1.7b_base_v2 \
    --data_path ./data/cpo_hymt7b_vs_ours.jsonl \
    --output_dir ./checkpoints/output_1.7b_cpo_7b_lora \
    --lr 1e-4 \
    --beta 0.05 \
    --nll_weight 1.0 \
    --batch_size 1 \
...
cpo_7b_full() {
  $PY -u train/train_cpo.py \
    --model_path ./checkpoints/output_1.7b_base_v2 \
    --data_path ./data/cpo_hymt7b_vs_ours.jsonl \
    --output_dir ./checkpoints/output_1.7b_cpo_7b_full \
    --lr 1e-5 \
    --beta 0.05 \
    --nll_weight 1.0 \
    --batch_size 1 \
